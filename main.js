...
class Lifx extends utils.Adapter {
	...
	async onReady() {
				...
				if (err) {
					...
					light.getState(async (err, state) => {
						if (err) {
							...
							await this.setStateAsync('Bulb_' + light.id + '.version', { val: info.version, ack: true });
							...
							await this.setStateAsync('Bulb_' + light.id + '.label', { val: state.label, ack: true });
							const convertPower = state.power == 1 ? true : false;
							await this.setStateAsync('Bulb_' + light.id + '.state', { val: convertPower, ack: true });
							await this.setStateAsync('Bulb_' + light.id + '.duration', { val: 500, ack: true });
							...
							await this.setStateAsync('Bulb_' + light.id + '.online', { val: true, ack: true }); // because we found the lamp
						}
		...
		client.on('light-online', async (light) => {
			...
			await this.setStateAsync('Bulb_' + light.id + '.online', { val: true, ack: true });
		});

		client.on('light-offline', async (light) => {
			this.log.info('Light offline. ID:' + light.id + ', IP:' + light.address + ':' + light.port + '\n');
			await this.setStateAsync('Bulb_' + light.id + '.online', { val: false, ack: true });
		});
		...
		client.lights().forEach(async (light) => {
			light.getState(async (err, info) => {
				if (err) {
					...
					await this.setStateAsync('Bulb_' + light.id + '.state', { val: convertPower, ack: true });
					await this.setStateAsync('Bulb_' + light.id + '.hue', { val: info.color.hue, ack: true });
					await this.setStateAsync('Bulb_' + light.id + '.sat', { val: info.color.saturation, ack: true });
					await this.setStateAsync('Bulb_' + light.id + '.bright', { val: info.color.brightness, ack: true });
					await this.setStateAsync('Bulb_' + light.id + '.temp', { val: info.color.kelvin, ack: true });
				}
			});
			light.getHardwareVersion(async (err, info) => {
				if (err) {
						...
						if (zone === null) {
							this.getState('Bulb_' + id + '.bright', (err, obj) => {
								client.light(id).color(0, 0, obj.val, state.val, dur.val, (err) => {
									...
									this.setStateAsync('Bulb_' + id + '.colormode', { val: 'white', ack: true });
								});
							});
						} else {
										...
										client
											...
											.colorZones(
												...
												(err) => {
													...
													if (err) {
														this.log.debug(
															'Coloring light ' + id + ' in zone ' + zone + ' failed'
														);
													}
													this.log.debug(
														'Coloring light ' +
										...
										client
											...
											.colorZones(
												...
												(err) => {
													...
													if (err) {
														this.log.debug(
															'Saturation light ' + id + ' in zone ' + zone + ' failed'
														);
													}
													this.log.debug(
														'Saturation light ' +
